...
`superstructure/Superstructure.java` is the central coordinator. States: `STOWED → PREPPING_TO_SHOOT → SHOOTING → EXHAUSTING → TRAVERSING_TRENCH`. Intake is fully operator-controlled and not managed by the Superstructure. It commands scoring subsystems (shooter, turret, feeder, spindexer) based on state transitions and sensor feedback (ball count, readiness flags, anti-jam).
